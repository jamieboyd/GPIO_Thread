...
last modified:
2018/08/06 by Jamie Boyd - modifyied for pulsedThread subclassing
...
int ptPWM_Init (void * initDataP, void *  volatile &taskDataP){
	...
	taskData->polarity = initData->polarity;
	// copy data settings
	taskData->arrayData= initData->arrayData;
	taskData->nData = initData->nData;
	taskData->startPos =0;
	taskData->arrayPos =0;
	taskData->endPos =taskData->nData ;
	// things that vary by chanel
	...
	// save control and data register address for easy access from thread function
	...
	*(initData->PWMaddr  + rangeRegisterOffset) = initData->range; 
	...
	if (initData->mode ==PWM_MARK_SPACE){
		*(initData->PWMaddr  + PWM_CTL) |= modeBit ; // put PWM in MS Mode
	}else{
		*(initData->PWMaddr  + PWM_CTL) &= ~modeBit;  // clear MS mode bit for balanced mode
	}
	// set polarity
	...
	if (initData->waitForEnable){
		*(taskData -> ctlRegister) &= ~(taskData->enableBit);
	}else{
		// set initial PWM value first so we have something to put out
		*(taskData->dataRegister) = taskData->arrayData[taskData->dataPos];
		*(taskData -> ctlRegister)|= taskData->enableBit;
	}
	return 0; // 
}
