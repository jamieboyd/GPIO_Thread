...
high and low from Python,with same code*/
...
static PyMethodDef ptSimpleGPIOMethods[] = {
	...
	{"setEndFuncObj", pulsedThread_SetPythonEndFuncObj, METH_VARARGS, "(PyCapsule, PythonObj) sets a Python object to provide endFunction for pulsedThread"},
	...
	{"output", simpleGPIO_output, METH_VARARGS, "(int pinNumber, int initial level) Creates a simple output on a single pin, with no associated thread"},
	{"setOutput", simpleGPIO_setOutput, METH_VARARGS, "(PyCapsule, int level) sets level of a simple output with no associated thread"},
	{ NULL, NULL, 0, NULL}
  };
